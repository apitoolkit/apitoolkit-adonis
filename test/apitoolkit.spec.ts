...
import { APIToolkitMiddleware } from '../src/Middleware/APIToolkitMiddleware.js'
import { setupApp, fs } from '../test-helpers.js'
...
test.group('APIToolkitMiddleware Test', (group) => {
  ...
  test('Should get data', async ({ assert }: any) => {
    ...
    assert.equal(response.status, 200)
    assert.equal(JSON.stringify(response.body), JSON.stringify(exampleRequestData))
    assert.equal(published, true)
  })
})
